...
// --- Assets & Icons (SVGs inside components for clean code) ---
...
const PaypalLogo = () => (
  <svg className="h-6 w-auto" viewBox="0 0 24 24" fill="currentColor" style={{color: '#003087'}}><path d="M7.076 21.337l.756-4.793h-1.564l-.756 4.793h1.564zm8.682-15.006c-.846-3.56-5.83-3.692-5.83-3.692H5.666L2.33 21.337h4.84l.87-5.513h2.646c4.697 0 7.828-2.296 8.783-6.666.257-1.177.17-2.18-.715-2.827z"/></svg>
);

const PayeerLogo = () => (
   <div className="flex items-center bg-gray-100 rounded px-2 h-5">
      <span className="text-[10px] font-black text-[#009473]">PAYEER</span>
   </div>
);

const ArrowRight = () => <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>;
const CheckCircle = () => <svg className="w-16 h-16 text-green-500 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
...
const DonatePage = () => {
  ...
  // Helper to get Icon based on method
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] font-sans flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 text-center mb-14">
        <h1 className="text-4xl md:text-5xl tracking-wider font-brush transform -rotate-2 drop-shadow-sm">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        {!isSuccess && (
          <div className="flex justify-between items-center mb-6 px-4">
             <div className="flex gap-2">
                {[1, 2, 3].map((s) => (
                    <div key={s} className={`h-1.5 rounded-full transition-all duration-500 ${step >= s ? 'w-8 bg-[#006233]' : 'w-2 bg-gray-300'}`}></div>
                ))}
             </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden min-h-[460px] relative flex flex-col transition-all duration-300">
          
          {isSuccess ? (
             <div className="flex-1 flex flex-col items-center justify-center p-8 text-center slide-in">
                <CheckCircle />
                <h2 className="text-3xl font-brush text-[#1e293b] mb-2">Chokran!</h2>
                <div className="bg-green-50 rounded-xl p-4 w-full mb-6 border border-green-100">
                    <p className="text-gray-500 text-sm">Amount Donated</p>
                    <p className="text-2xl font-bold text-[#006233]">${donationAmount}</p>
                ...
                <Link href="/" className="w-full bg-[#1e293b] hover:bg-black text-white px-6 py-4 rounded-xl font-bold transition shadow-lg">
                   Return to Website
                </Link>
             </div>
          ) : (
                ...
                {step === 1 && (
                  <div className="slide-in flex-1 flex flex-col justify-center">
                     ...
                     <div className="relative mb-6 group">
                        <span className="absolute left-6 top-1/2 -translate-y-1/2 text-3xl font-serif text-gray-300 transition-colors group-focus-within:text-[#006233]">$</span>
                        <input
                            type="number"
                            value={donationAmount}
                            onChange={handleCustomAmount}
                            placeholder="0"
                            className="w-full pl-12 pr-4 py-6 bg-gray-50 border-2 border-transparent focus:border-[#006233] focus:bg-white rounded-2xl outline-none text-2xl font-bold text-[#1e293b] text-center transition-all placeholder-gray-200 shadow-inner"
                        />
                     </div>
                  ...
                  <div className="slide-in flex-1 flex flex-col">
                     ...
                     <div className="bg-[#f0fdf4] border border-[#dcfce7] rounded-xl p-3 mb-6 flex justify-between items-center animate-pulse-once">
                        <div className="flex items-center gap-2">
                            <div className="bg-[#006233] text-white rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold">✓</div>
                            <span className="text-sm text-[#006233] font-medium">Donating: <span className="font-bold text-lg ml-1">${donationAmount}</span></span>
                        </div>
                     ...
                     <div className="space-y-3">
                        {['card', 'paypal', 'payeer'].map((method) => (
                            <label key={method} className={`relative flex items-center justify-between p-4 border-2 rounded-2xl cursor-pointer transition-all duration-200 group ${
                                paymentMethod === method 
                                ? 'border-[#006233] bg-white shadow-md' 
                                : 'border-gray-50 bg-gray-50 hover:bg-white hover:border-gray-200'
                            }`}>
                                <div className="flex items-center gap-4">
                                    <input type="radio" name="payment" value={method} checked={paymentMethod === method} onChange={(e) => setPaymentMethod(e.target.value)} className="w-5 h-5 accent-[#006233]" />
                ...
                {step === 3 && (
                   <div className="slide-in flex-1 flex flex-col justify-center">
                      ...
                      <div className="bg-white border-2 border-dashed border-gray-200 rounded-2xl p-6 relative">
                          <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#FDFBF7] px-3 text-xs font-bold text-gray-400 uppercase tracking-widest">
                             Summary
                          </div>
                          ...
                          <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-100">
                             ...
                             <span className="text-3xl font-bold text-[#006233] font-serif">${donationAmount}</span>
                          </div>
                          ...
                          <div className="flex justify-between items-center">
                             ...
                             <div className="flex items-center gap-2">
                                <span className="text-sm font-bold text-[#1e293b] capitalize">{paymentMethod === 'card' ? 'Credit Card' : paymentMethod}</span>
                      ...
                      <div className="mt-6 flex items-center justify-center gap-2 text-[#006233] bg-[#006233]/5 p-3 rounded-xl">
                          ...
                          <span className="text-xs font-bold uppercase tracking-wide">SSL Secure Encrypted</span>
                      </div>
                   </div>
                )}
              ...
              <div className="p-6 pt-0 mt-auto">
                 {step === 3 ? (
                    <button
                        ...
                        disabled={isLoading}
                        className="w-full bg-[#C1272D] hover:bg-[#a01f24] text-white py-4 rounded-xl font-bold text-lg shadow-xl shadow-red-900/10 transition-all duration-300 flex justify-center items-center gap-2 transform active:scale-95"
                    >
                        {isLoading ? (
                    ...
                    <button
                        ...
                        className="w-full bg-[#1e293b] hover:bg-black text-white py-4 rounded-xl font-bold text-lg shadow-lg transition-all duration-300 flex justify-center items-center gap-2 group"
                    >
                        Continue 
                 ...
                 {step > 1 && (
                    <button onClick={prevStep} className="w-full text-center text-gray-400 text-sm font-bold mt-4 hover:text-gray-600 transition">
                        Go Back
                    </button>
                 )}
              </div>
